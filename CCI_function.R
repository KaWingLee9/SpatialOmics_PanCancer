...
CalSampleCCI <- function(lr_score_spot,cell_type_score,
                         p.adjust.method='bonferroni'){
    
    x=cbind()
    pearson_result=Hmisc::rcorr(as.matrix(cell_type_score),lr_score_spot)
...
CalLCPCCI <- function(lr_score_spot,cell_type_score,MLCP_result,cor_result,
        ...
        test_result=sapply(obs_LCP,function(obs_LCP){
            ...
            x=sapply(colnames(cell_type_score_tmp),function(i){
                ...
                lr_score_spot_tmp=lr_score_spot_tmp[,filter(cor_result,cell_type==i) %>% .[,'lr_pair']]
